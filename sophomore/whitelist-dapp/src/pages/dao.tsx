...
export default function Home() {
  ...
  const voteOnProposal = async (proposalId: number, _vote: 'YAY' | 'NAY') => {
    try {
      const signer = await getProviderOrSigner(true);
      const daoContract = getDaoContractInstance(signer);
  ...
  const getDaoContractInstance = (providerOrSigner: providers.Web3Provider | providers.JsonRpcSigner) => {
    return new Contract(
      DAO_CONTRACT_ADDRESS,
      cryptoDevDaoContractAbi,
      providerOrSigner
