...
export default function Home() {
  ...
  const getAmounts = async () => {
    try {
      ...
      const _ethBalanceContract = await getEtherBalance(provider, undefined, true);
  ...
  const _getAmountOfTokensReceivedFromSwap = async (_swapAmount: string) => {
    try {
      ...
      if (!_swapAmountWEI.eq(zero)) {
        ...
        const _ethBalance = await getEtherBalance(provider, undefined, true);
  ...
  return (
    <div>
      ...
      <AppContainer>
        ...
        <div>
          <img className={styles.image} src="./cryptodevs/cryptodev.svg" />
        </div>
      </AppContainer>
    </div>
  );
